...
$(document).ready(function () {
    ...
    //  end slide
    $('#view-more-product-btn').on('click', function () {
        handleClickAddContainerProduct()
    })
    function handleClickAddContainerProduct() {
        let checkExist = !($('#add-container-product').html()?.trim() == '')
        if (!checkExist) {
            let productHTML = $('#container-product-example').html()
            $('#add-container-product').append(`
            <div class="product__detail">
            ${productHTML}
            </div>
            `)
            $('#view-more-product-btn').html('View smaller →')
        }
        else {
            $('#add-container-product').empty()
